...
api_id = "28174104"
api_hash = "95172f0d86b38b257e160a7076a58e6f"
bot_token = "7291907858:AAEU1-AjvebB8PkprpEYMsdd6D5HLU4Y73o"
...
def generate_cards(bin,
    ...
    while len(cards) < count:
        try:
            card_number = bin + str(
                random.randint(
                    0, 10**(16 - len(bin) - 1) - 1)).zfill(16 - len(bin))
            if luhn_check(card_number):
                expiry_date = generate_expiry_date(current_year, current_month,
...
def generate_expiry_date(current_year,
                         current_month,
                         expiry_month=None,
                         expiry_year=None):
    month = str(expiry_month if expiry_month and expiry_month != 'xx' else
                random.randint(1, 12)).zfill(2)
    year = str(expiry_year if expiry_year and expiry_year != 'xx' else random.
               randint(current_year, current_year + 5)).zfill(2)
...
def start(message):
    help_text = ("""
    Welcome This is a Cc generator bot! :-

/generate 123456 10
/generate 123456xxxx|xx|xx|xxx 

to create visa use these
/gen bin
/gen 123456
    ...
    markup.add(
        InlineKeyboardButton("Main Channel", url="https://t.me/wgxscrape"))
    bot.send_photo(message.chat.id,
                   "https://images.app.goo.gl/V1ppNX1ce5sXWcaw6",
                   caption=help_text,
                   reply_markup=markup)
...
def generate(message):
    try:
        ...
        if count > 100000:
            bot.reply_to(message, "Maximum Number of cards is 100,000.")
            return
        ...
        additional_info = (
            ...
            f"Scraped By: [{user_info.first_name}]\n\n"
            f"𝗜𝗻𝗳𝗼: {bin_info['scheme']} - {bin_info['type']} - {bin_info['brand']}\n"
            f"𝐈𝐬𝐬𝐮𝐞𝐫: {bin_info['bank']}\n"
            f"𝗖𝗼𝘂𝗻𝘁𝗿𝘆: {bin_info['country']} {bin_info['emoji']}")

        with open(file_path, "rb") as file:
            bot.send_document(message.chat.id, file, caption=additional_info)
        add_reaction(message)
    except Exception as e:
        bot.reply_to(
            message,
            f"an error occurred:-\n `{e}`\n\n Contact the developer to solve the problem :- @lordwg_xd")
...
def add_reaction(message):
    try:
        ...
        bot.reply_to(message, f"An error occurred while adding the interaction: {e}")
